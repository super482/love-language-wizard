...
import { generateConversationStarter } from '@/utils/aiUtils';
  ...
  const handleGenerate = async () => {
    ...
    try {
      const starter = await generateConversationStarter({
        imageDescription: "A person smiling outdoors", // In a real app, this would be from image analysis
        tone: selectedTone,
        context: context || undefined
      });
      
      setGeneratedStarter(starter);
    } catch (error) {
      ...
      <main className="flex-1 container mx-auto px-4 py-6">
        ...
        <div className="glass-card rounded-2xl p-6 mb-8">
          <h2 className="text-xl text-center mb-4">
            Upload a pic of a person or activity for conversation starters
          </h2>
          
          <div className="mb-6">
            <IconUpload
              onImageSelect={handleImageSelect}
              icon={<Image className="text-magic" />}
              text="Add pic of person"
            />
          </div>
        ...
        {generatedStarter && (
          <div className="glass-card rounded-2xl p-6 mb-8 animate-accordion-down">
            ...
            <div className="bg-secondary/30 p-4 rounded-lg border border-white/10">
              <p className="text-lg">{generatedStarter}</p>
            </div>
